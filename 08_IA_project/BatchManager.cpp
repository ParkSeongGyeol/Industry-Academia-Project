...
void BatchManager::addBatch() {
    ...
    cin.ignore();
    cout << "원재료 조성 (예: RM001:60%,RM002:40%): ";
...
void BatchManager::updateBatch() {
    ...
    for (auto& batch : batches) {
        if (batch.getBatchId() == id) {
            ...
            cout << "완료일 (" << batch.getEndDate() << "): ";
            cin >> input; batch.setEndDate(input);

            cout << "유통기한 (" << batch.getExpiryDate() << "): ";
            ...
            cin.ignore();
            cout << "원재료 조성 (" << batch.getIngredientInfo() << "): ";
